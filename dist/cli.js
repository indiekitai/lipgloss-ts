  ...
  for (const l of lines) {
    ...
    if (shortAmount > 0) {
      if (pos === Right || pos === 1) {
        ...
        const left2 = Math.floor(shortAmount / 2);
        const right2 = shortAmount - left2;
        const ls = " ".repeat(left2);
        const rs = " ".repeat(right2);
        result.push(
...
function renderHorizontalEdge(left2, middle, right2, w) {
  if (!middle) middle = " ";
  const leftW = stringWidth(left2);
  const rightW = stringWidth(right2);
  const runes = [...middle];
  let out = left2;
  let j = 0;
...
var Style = class _Style {
  ...
  borderForeground(...colors2) {
    if (colors2.length === 0) return this;
    const sides = whichSides(colors2);
    if (!sides) return this;
    const s = this._clone();
  ...
  _applyBorder(str) {
    ...
    let out = "";
    if (hasTop) {
      const top2 = renderHorizontalEdge(border.topLeft, border.top, border.topRight, totalWidth);
      out += this._styleBorder(top2, this._borderTopFg, this._borderTopBg) + "\n";
    }
    ...
    if (hasBottom) {
      const bottom2 = renderHorizontalEdge(border.bottomLeft, border.bottom, border.bottomRight, totalWidth);
      out += "\n" + this._styleBorder(bottom2, this._borderBottomFg, this._borderBottomBg);
    }
    return out;
  }
  _applyMargins(str) {
...
function JoinHorizontal(pos, ...strs) {
  ...
  for (let i = 0; i < blocks.length; i++) {
    ...
    if (pos === Top || pos === 0) {
      ...
      const top2 = extra - split;
      blocks[i] = [
        ...empties.slice(0, top2),
        ...blocks[i],
        ...empties.slice(0, extra - top2)
      ];
    }
  }
  const result = [];
    ...
    for (const line of block) {
      ...
      if (pos === Left || pos === 0) {
        ...
        if (w < 1) {
          ...
          const right2 = w - split;
          const left2 = w - right2;
          result.push(" ".repeat(left2) + line + " ".repeat(right2));
        }
      }
    }
  }
...
console.log("\n\u{1F3A8} @indiekit/lipgloss - Terminal Styling Demo\n");
var title = NewStyle().bold().foreground("#FF6B6B").render("Bold Red Title");
var subtitle = NewStyle().italic().faint().foreground("#888888").render("Italic faint subtitle");
console.log(title);
console.log(subtitle);
console.log();
var box = NewStyle().border("rounded").borderForeground("#874BFD").padding(1, 3).width(40).align(Center).foreground("#FAFAFA").background("#2D2D2D").render("Hello from lipgloss-ts!\nCSS-like terminal styling");
console.log(box);
console.log();
var borderStyles = ["normal", "rounded", "double", "thick", "hidden"];
for (const name of borderStyles) {
  const b = NewStyle().border(name).borderForeground("#00FA68").padding(0, 1).render(name);
  process.stdout.write(b + "  ");
}
console.log("\n");
var left = NewStyle().border("rounded").borderForeground("#FF6B6B").padding(1, 2).width(24).align(Center).render("Left Panel");
var right = NewStyle().border("rounded").borderForeground("#874BFD").padding(1, 2).width(24).align(Center).render("Right Panel");
console.log("Horizontal Join (top-aligned):");
console.log(JoinHorizontal(Top, left, right));
console.log();
var top = NewStyle().border("normal").borderForeground("#00FA68").padding(0, 2).width(50).align(Center).render("Top Section");
var bottom = NewStyle().border("double").borderForeground("#FF6B6B").padding(0, 2).width(50).align(Center).render("Bottom Section");
console.log("Vertical Join:");
console.log(JoinVertical(Left, top, bottom));
console.log();
var colors = ["#FF6B6B", "#4ECDC4", "#FFE66D", "#95E1D3", "#F38181", "#874BFD"];
var palette = "";
...
console.log(palette);
console.log();
var listStyle = NewStyle().border("rounded").borderForeground("#874BFD").padding(1, 2).width(40);
var items = ["\u2705 Chainable API", "\u{1F3A8} True Color support", "\u{1F4E6} Zero dependencies", "\u{1F4D0} Layout utilities"];
var list = items.join("\n");
